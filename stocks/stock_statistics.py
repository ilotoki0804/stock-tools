...
def CAGR(states: list[State] | pd.DataFrame) -> float:
    ...
    years_diff: float = (getter(last_day, 'date') - getter(first_day, 'date')).days / 365

    return total_earning_multiple ** (1 / years_diff) - 1


def stock_volatility(broker: mojito.KoreaInvestment, company_code: str, date_type: Literal['D', 'W', 'M'], start_day: datetime, end_day: datetime, price_from: str = 'open'):
    """시작일과 종료일 모두를 포함합니다. 주의: 이 함수는 버그가 있을 수 있습니다."""

    price_change_rates = []
    prev_single_price = None
    for price in fetch_prices_by_datetime(broker, company_code, date_type, start_day, end_day):
        single_price = int(price[SIGNIFICANT_PRICE_NAMES[price_from]])

        if prev_single_price is not None:
            print(f"{single_price - prev_single_price=}, {single_price=}, {(single_price - prev_single_price) / single_price=}")
