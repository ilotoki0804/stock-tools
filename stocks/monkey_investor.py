...
def monkey_investor(
    ...
    start_day: datetime,
    end_day: datetime,
    invest_amount: tuple[Annotated[float, 'counts'], Annotated[float, 'standard_deviation']],
    total_invest_count: int,
    seed: int | None = None,
) -> tuple[PriceCache, list[Transaction], State, datetime]:
    """
    Args:
        end_day: 마지막 날을 의미합니다. 당일을 포함합니다.
    ...
    day_range = range((start_day - standard_day).days, (end_day - standard_day).days + 1)

    total_amount = 0
    transactions: list[Transaction] = []
    for transaction_day_diff in sorted(seeded_random.choices(day_range, k=total_invest_count)):
        ...
        transaction_amount = round(seeded_random.normalvariate(*invest_amount) * buy_or_sell)
        ...
        transaction_price = seeded_random.randint(
            int(price[SIGNIFICANT_PRICE_NAMES['low']]), int(price[SIGNIFICANT_PRICE_NAMES['high']]))

        transactions.append(Transaction(transaction_day, company_code, transaction_amount, transaction_price))
